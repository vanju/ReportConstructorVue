<script>
    ...
    import MenuBuilderProvider from '/src/libs/order-menu-builder.js';
    import ExpressionNodeOrderByContainer from './ExpressionNodeOrderByContainer.vue';

    export default {
        name: 'OrderByExpressionEditor',
        extends: ExpressionEditor,
        components: {
            ExpressionEditor: ExpressionEditor,
            ExpressionNodeOrderByContainer: ExpressionNodeOrderByContainer
        },
        props: {
            queryId: {
                type: Number,
        ...
        async created() {
            let response = await this.$fetch('/expression-node/tree/?id=' + this.rootId + '&clause=orderby', { method: 'GET', mock: false });
    ...
    <div v-if="isLoaded" class="context-menu-wrapper" ref="editor">
        ...
        <div class="fl-row">
            <div class="expression-node-tree">
                <expression-node-order-by-container v-bind:init-expression-node="model.ExpressionNode"
                                                    v-bind:key="treeVersion"
                                                    v-bind:parent-id="model.ExpressionNode.ParentId"
                                                    >
                </expression-node-order-by-container>
            </div>
